...
<head>
    ...
    <style>
        ...
        .message.other {
            ...
            text-align: right;
            margin-right: 330px; /* Align to the left */
            width: fit-content;
            max-width: 70%;
        }
    </style>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header card-header-primary">
                                    <h3 class="card-title" align="center"><b>Chat with Wholesaler</b></h3>
                                    <p class="card-category" align="center"><b>Communicate with your Wholesaler</b></p>
                                </div>
                                    <div class="card-body">
                                    <div class="chat-container">
									<h3 align="center"><b>Chat Between <%= session.getAttribute("Pharmacyname") %> 
									    	& Wholesaler - <%= request.getParameter("Wholersalername") %></b></h3>
		                                  <div class="chat-messages" id="chatMessages">
		                                    
		                                <div class="chat-container">

			                            <h5><b>Chat with WholeSeller <u>Mrs.<%=Wholersalername%></u></b></h5>
			                            
			                            <div class="chat-container">
										<%
											Connection con = Dbconn.conn();
								            String wid = request.getParameter("wid");
								            Pharmacyname = session.getAttribute("Pharmacyname").toString();
								            Wholersalername = request.getParameter("Wholersalername");
								
								            int pid = 0; 
								            PreparedStatement pst = con.prepareStatement("SELECT id FROM tblpharmacy WHERE Name=?");
								            pst.setString(1, Pharmacyname);
								            ResultSet rs = pst.executeQuery();
								            if (rs.next()) { pid = rs.getInt("id"); }
								
								            PreparedStatement chatStmt1 = con.prepareStatement("SELECT sender, message, timestamp FROM tblchat WHERE pid=? AND wid=? ORDER BY timestamp ASC");
								            chatStmt1.setInt(1, pid);
								            chatStmt1.setInt(2, Integer.parseInt(wid));
									    ...
									    <div class="chat-messages" id="chatMessages">
									        <% 
									            while (chatRs1.next()) {
									                String sender = chatRs1.getString("sender");
									                String message = chatRs1.getString("message");
									                String timestamp = chatRs1.getString("timestamp");
									        %>
									            <div class="message user <%= sender.equals(Pharmacyname) ? "user" : "other" %>">
									                <strong><b><%= sender %>:</b></strong> <%= message %>
									                <span style="font-size: small; color: gray;"><%= timestamp %></span>
									            </div>
									        <% } 
									        	PreparedStatement chatStmt = con.prepareStatement("SELECT receiver, message, timestamp FROM tblchat1 WHERE pid='"+pid+"' AND wid='"+wid+"' ORDER BY timestamp ASC");
								                //System.out.println("Message= "+ chatStmt);
								            	ResultSet chatRs = chatStmt.executeQuery();
								            	while (chatRs.next()) {
									                String receiver = chatRs.getString("receiver");
									                String message = chatRs.getString("message");
									                String timestamp = chatRs.getString("timestamp");
									        %>
									        
									        	<div class="message other <%= receiver.equals(Wholersalername) ? "user" : "other" %>">
										...
										<%
											...
											String wid = request.getParameter("wid");
											Pharmacyname = session.getAttribute("Pharmacyname").toString();
											PreparedStatement pst=con.prepareStatement("SELECT Name, id FROM tblpharmacy");
											ResultSet rs11=pst.executeQuery();
											while(rs11.next())
											{
											int pid = rs11.getInt("id");
											System.out.println("PharmaId= "+pid);
											Pharmacyname = rs11.getString("Name");
										%>
			                            <form action="ChatBot" method="post">
			                                <input type="text" name="chat" class="chat-input" placeholder="Type your message here" required>
			                                <input type="hidden" name="Rdate" value="<%=sdf.format(cal.getTime())%>" readonly="readonly">
			                                <input type="hidden" name="Pharmacyname" value="<%=Pharmacyname%>">
			                                <input type="hidden" name="Wholersalername" value="<%=Wholersalername%>">
			                                <input type="hidden" name="pid" value="<%=pid%>">
			                                <input type="hidden" name="wid" value="<%=wid%>">
			                    ...
			                    <%
			                    try {			                        
			                        PreparedStatement ps= con.prepareStatement("SELECT * FROM tblchat1 WHERE pid='"+pid+"' and wid='"+wid+"' ORDER BY Rdate");
			                        ...
			                        while(rs.next())
									{
			                            ...
			                            String rdateFromDB = rs.getString("Rdate");
			                        %>
			                            <div class="message user">
			                                <strong><%=session.getAttribute("Pharmacyname")%>: <%= chat %></strong>
			                                <span class="timestamp"><%= rdateFromDB %></span>
			                            </div>
			                        <%
			                        }
			                        
			                        PreparedStatement ps1= con.prepareStatement("SELECT * FROM tblchat WHERE pid='"+pid+"' and wid='"+wid+"' ORDER BY Rdate");
			                        ...
			                        while(rs1.next())
									{
			                            ...
			                            System.out.println("Msg= "+chat);
			                            String rdateFromDB = rs1.getString("Rdate");
			                        %>
			                            <div class="message other">
			                                <strong>Mr. <%=rs.getString("Pharmacyname")%>: <%= chat %></strong>
			                                <span class="timestamp"><%= rdateFromDB %></span>
			                            </div>
			                        <%
			                        }
